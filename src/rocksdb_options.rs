...
use merge_operator::{
    self, full_merge_callback, partial_merge_callback, MergeOperands,
...
impl BlockBasedOptions {
    ...
    pub fn set_index_type(&mut self, index_type: IndexType) {
        let it = match index_type {
            IndexType::BinarySearch => {
                rocksdb_ffi::BLOCK_BASED_INDEX_TYPE_BINARY_SEARCH
            }
            IndexType::HashSearch => {
                rocksdb_ffi::BLOCK_BASED_INDEX_TYPE_HASH_SEARCH
            }
        };
        unsafe {
            rocksdb_ffi::rocksdb_block_based_options_set_index_type(
                self.inner, it,
            );
        }
    }
    ...
    pub fn set_filter(&mut self, bits: i32) {
        unsafe {
            let new_filter =
                rocksdb_ffi::rocksdb_filterpolicy_create_bloom(bits);
...
impl Options {
    ...
    pub fn set_prefix_extractor_fixed_size<'a>(&mut self, size: usize) {
        unsafe {
            let st =
                rocksdb_ffi::rocksdb_slicetransform_create_fixed_prefix(size);
    ...
    pub fn set_db_write_buffer_size(&mut self, size: size_t) {
        unsafe {
            rocksdb_ffi::rocksdb_options_set_db_write_buffer_size(
                self.inner, size,
            );
        }
    }
    ...
    pub fn set_parsed_options(&mut self, opts: &str) -> Result<(), String> {
        unsafe {
            ...
            rocksdb_ffi::rocksdb_get_options_from_string(
                self.inner,
                c_opts_ptr as *const _,
                new_inner_options,
                err_ptr,
            );
            if !err.is_null() {
            ...
            rocksdb_ffi::rocksdb_options_destroy(mem::replace(
                &mut self.inner,
                new_inner_options,
            ));
            Ok(())
...
impl Cache {
    pub fn new(bytes: usize) -> Cache {
        Cache {
            inner: unsafe { rocksdb_ffi::rocksdb_cache_create_lru(bytes) },
        }
    }
}
...
impl Drop for Cache {
    fn drop(&mut self) {
        unsafe {
            rocksdb_ffi::rocksdb_cache_destroy(self.inner);
        }
    }
}
